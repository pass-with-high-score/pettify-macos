...
class AppState: ObservableObject {
    ...
    func updateCurrentLyric() {
        let pos = status.position - lyricsOffset
        var bestIndex = -1
        for (i, line) in lyrics.enumerated() {
            if line.time <= pos + 0.8 { // increased pre-fetch offset so lyrics appear earlier
                bestIndex = i
            } else {
                break
            }
        ...
        // Hide lyrics if we've passed the last lyric line by more than 20 seconds
        if bestIndex == lyrics.count - 1, let lastTime = lyrics.last?.time {
            if pos > lastTime + 20.0 {
                bestIndex = -1
            }
        }
