            ...
            VStack(alignment: state.isLeft ? .leading : .trailing, spacing: 8) {
                ...
                HStack(spacing: 20) {
                    ...
                    if isHovering {
                        HStack(spacing: 4) {
                            Button(action: { 
                                state.lyricsOffset -= 1.0
                                state.updateCurrentLyric()
                            }) {
                                Image(systemName: "minus.square").font(.system(size: 12))
                            }.buttonStyle(.plain)
                            Text("\(state.lyricsOffset > 0 ? "+" : "")\(String(format: "%.1f", state.lyricsOffset))s")
        ...
        Group {
            if imageURL.hasPrefix("file://"), let url = URL(string: imageURL), let nsImage = NSImage(contentsOf: url) {
                Image(nsImage: nsImage)
                    ...
                    .scaleEffect(imageURL.contains("youtube.com") ? 1.35 : 1.0)
            } else {
